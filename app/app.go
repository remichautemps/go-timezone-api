...
import (
	...
	"github.com/remichautemps/errorlog/logger"
	"log"
	"net/http"
	"os"
	"strings"
...
func Start() {
	...
	logger.Info(fmt.Sprintf("Starting our application on %s:%s", address, port))
...
func getTime(w http.ResponseWriter, r *http.Request) {
	...
	for _, tz := range strings.Split(timezones, ",") {
		...
		if strings.Contains(allowedTimezones, tz) {
			...
			respTimes[tz] = time.Now().In(loc).String()
		} else {
			e := appError{
	...
	// if we couldn't find a timezone, return a default timezone
	if len(respTimes) == 0 {
		respTimes["current_time"] = time.Now().UTC().String()
	}
